...
let run_no_vg (program_name : string) args std_input : (string, string) result =
  ...
  let result =
    ...
    | WSTOPPED n ->
      Error (sprintf "Stopped with signal %d while running %s." n program_name)
  in
...
let test_doesnt_run filename test_ctxt =
  ...
  | Ok unexpected ->
    assert_failure
      (sprintf "Expected program to fail, but it didn't:\nReceived: %s" unexpected)
  | Error _ ->
...
let test_doesnt_err filename test_ctxt =
  ...
  match result with
  | Ok _ ->
    assert_bool (sprintf "Program %s currently runs (as expected for now)" filename) true
  | Error errmsg ->
    assert_failure
      (sprintf "Expected program to succeed, but it didn't:\nReceived: %s" errmsg)
;;
...
let input_file_test_suite () =
  ...
  >::: [ "do_pass"
         >::: List.map
                (fun f -> f >:: test_does_run f)
                (safe_readdir "input/do_pass" ".garter")
       ; "do_err"
         >::: List.map
                (fun f -> f >:: test_does_err f)
                (safe_readdir "input/do_err" ".garter")
       ; "dont_pass"
         >::: List.map
                (fun f -> f >:: test_doesnt_run f)
                (safe_readdir "input/dont_pass" ".garter")
       ; "dont_err"
         >::: List.map
                (fun f -> f >:: test_doesnt_err f)
                (safe_readdir "input/dont_err" ".garter")
       ]
;;
