...
let rec lookup (funname : string) (name : string) (env : naive_stack_env) =
  ...
  | Some (_, name_env) ->
    ...
    | None ->
      ...
      then raise (InternalCompilerError (sprintf "failed to lookup name %s " name))
...
let get_next_function (expr : tag cexpr) : string =
  match expr with
...
and allocate_cexpr
  ...
  | CLambda (args, body, tag) ->
    ...
    let frees = StringSet.elements (StringSet.remove rec_name (fv_C expr)) in
    let frees_env =
...
let naive_stack_allocation (prog : tag aprogram) : tag aprogram * arg name_envt name_envt =
  match prog with
  | AProgram (expr, tag) ->
    let exp_envt = allocate_aexpr expr "closure#0" 1 [ "closure#0", [] ] in
